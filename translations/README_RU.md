...
> [!WARNING]
> **ПРЕДУПРЕЖДЕНИЕ О СБОРКЕ:** Хотя **TCC** и **Zig CC** отлично работают в качестве бэкендов для ваших программ Zen C, сборка *самого компилятора Zen C* с их помощью может пройти проверку, но создать нестабильный бинарный файл, который не пройдет тесты. Мы рекомендуем собирать компилятор с помощью **GCC** или **Clang** и использовать TCC/Zig только как бэкенд для вашего рабочего кода.
